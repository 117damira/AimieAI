...
import { TOPIK_READING_CONTENT_BANK } from "@/config/topik-reading-content";
import { TOPIK_READING_LEVELS } from "@/config/topik-reading";

/**
 * Content rotation for TOPIK Reading — direct copy of
 * lib/reading/rotation.ts's established pattern. Practice sessions avoid
 * repeating the most-recently-completed passage(s); the Daily Challenge is
 * a pure function of level + calendar date (no per-user state), so every
 * student at the same level sees the same set on the same day with no
 * server or shared storage required. Math.random() is fine here — this is
 * content rotation, never scoring.
 */
...
export function pickNextTopikPassageIds(
  level: TopikLevel,
  mode: Exclude<TopikReadingMode, "daily-challenge">,
  history: string[]
): string[] {
  const pool = TOPIK_READING_CONTENT_BANK[level];

  if (mode === "full-exam") {
    const examSize = Math.min(TOPIK_READING_LEVELS[level].passageCountMin, pool.length);
 ...
 * needed. */
export function pickTopikDailyChallengePassageIds(level: TopikLevel, date: Date = new Date()): string[] {
  const pool = TOPIK_READING_CONTENT_BANK[level];
 ...
 * otherwise exhausted. */
export function recordTopikReadingHistory(history: string[], passageIds: string[]): string[] {
