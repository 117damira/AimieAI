import type { TopikLevel, TopikListeningMode } from "@/types/topik-listening";
import { TOPIK_LISTENING_CONTENT_BANK } from "@/config/topik-listening-content";
import { TOPIK_LISTENING_LEVELS } from "@/config/topik-listening";

/**
 * Content rotation for TOPIK Listening — mirrors lib/listening/rotation.ts's
 * established pattern exactly. Practice sessions avoid repeating the
 ...
 * shared storage required. Math.random() is fine here — it only picks
 * which content to show next, it never scores anything.
 */
 ...
 * - "full-exam": a rotating subset sized to the level's recording count
 *   (never the whole pool at once, so two consecutive exams can genuinely
 *   differ once the pool is larger than that count), ordered so
 *   never-completed recordings are preferred. */
export function pickNextTopikRecordingIds(
  level: TopikLevel,
  mode: Exclude<TopikListeningMode, "daily-challenge">,
  history: string[]
): string[] {
  const pool = TOPIK_LISTENING_CONTENT_BANK[level];

  if (mode === "full-exam") {
    const examSize = Math.min(TOPIK_LISTENING_LEVELS[level].recordingCountMin, pool.length);
...
export function pickDailyTopikChallengeRecordingIds(level: TopikLevel, date: Date = new Date()): string[] {
  const pool = TOPIK_LISTENING_CONTENT_BANK[level];
 ...
 * otherwise exhausted. */
export function recordTopikListeningHistory(history: string[], recordingIds: string[]): string[] {
