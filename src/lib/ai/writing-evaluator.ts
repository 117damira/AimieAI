...
1. Task completion: does the essay genuinely address the actual prompt given (not just meet a word count)? Set "addressedPrompt" accordingly, and explain in "notes" — if it's off-topic, meaningless, or too short, say so plainly; if it's on-topic, confirm what it covers. List concrete missing pieces in "missingElements" (e.g. "a greeting", "your age", "a conclusion") — empty array if genuinely nothing is missing. "respectedFormat" reflects whether the word count (currently ${wordCount}, target ${minWords}-${maxWords}) is reasonably in range. Be strict: if the student did not actually answer what was asked, this must be reflected as a serious failure, not a minor deduction.
...
3. Structure: does it have an appropriate introduction, clearly organized main ideas, and (since this level ${conclusionRequired ? "requires" : "does not require"} one) a conclusion? Set "hasIntroduction"/"hasMainIdeas"/"hasConclusion" strictly based on what is ACTUALLY present in the text — never mark a conclusion (or any section) as present just because the level doesn't require one, or by default. Absence must show as absence. Set "conclusionRequired" (${conclusionRequired}) and describe the paragraph organization honestly in "paragraphOrganization".
4. Coherence: do ideas connect logically from sentence to sentence (real connectors, real logical flow), as opposed to just being present (that's structure's job)? Never default to a high coherence rating — random, disconnected, or meaningless text must be rated low, even if it technically has no punctuation errors. Set "isCoherent" and explain in "notes".
...
7. Exam readiness: compute "estimatedScore" (out of 25, "scoreOutOf": 25) from genuine, strict judgment of the above, following real DELF assessment standards — never a fixed baseline with jitter, and never generous by default. Meaningless, random, or completely off-topic writing (e.g. keyboard mashing, unrelated word lists) must score close to 0, not a moderate/middling number — a low score must actually look low. List 2-4 real "strengths" (empty if the prompt wasn't genuinely addressed) and "weaknesses" and "improvementTips" specific to this essay. Explain the score honestly in "scoreExplanation", including why points were lost.
8. Improved version: write "improvedVersion" — a genuinely unique rewrite generated fresh from THIS student's own writing: preserve their actual ideas, correct grammar, improve vocabulary and fluency where it's safe to do so without changing meaning, improve structure, and naturally expand on what they wrote. If something structural is missing (greeting, closing, more content), use natural scaffolding phrasing or a clearly bracketed placeholder telling the student what to add (e.g. "[Add here: your age, your hobbies]") — vary your phrasing essay to essay rather than reusing the same fixed wording every time. Never invent personal facts, opinions, or details the candidate never wrote — only improve what they actually wrote and mark genuine gaps. Never output a generic template; this must read as specific to this exact submission.
