...
import { getAllTopikDefinitions } from "@/lib/mock/topik-word-of-the-day";

/**
 * TOPIK equivalent of lib/quiz/generateQuiz.ts — builds the Weekly Quiz
 * entirely from Korean vocabulary the student has actually practiced
 * (profile.topikVocabularyProgress), never a static/preloaded question
 * list. A brand-new TOPIK account with no practiced words gets zero
 * questions; the page shows an honest empty state instead of demo content.
 * Fully independent of the DELF generator — never mixes word data.
 */
...
export function generateTopikVocabularyQuiz(
  ...
  const fillerPool = getAllTopikDefinitions(language).filter((d) => !practicedDefinitions.includes(d));

  const entries = seededShuffle(vocabularyProgress, hashString("topik-quiz-order")).slice(0, MAX_QUESTIONS);
