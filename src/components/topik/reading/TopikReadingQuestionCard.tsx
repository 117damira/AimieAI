...
import type { TopikReadingQuestion } from "@/types/topik-reading";

const DIFFICULTY_EMOJI = { easy: "🟢", medium: "🟡", hard: "🔴" } as const;

/**
 * Mirrors ReadingQuestionCard exactly. Selecting an option only ever marks
 * it as selected — correctness is NEVER revealed here (real-exam
 * behavior). The difficulty badge is always visible. The hint is
 * collapsed by default and, when expanded, shows only strategy-only
 * guidance — never the answer.
 */
export function TopikReadingQuestionCard({
  question,
  questionNumber,
  selectedOptionIds,
  onToggle,
