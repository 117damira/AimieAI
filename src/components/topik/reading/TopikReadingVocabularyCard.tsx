...
import type { TopikReadingVocabularyItem } from "@/types/topik-reading";

/**
 * Mirrors ReadingVocabularyCard exactly. "New Vocabulary" — every word
 * comes straight from the generated TopikReadingSet.vocabulary (extracted
 * from the actual Korean passage(s), never invented independently). "Save
 * to Vocabulary" is deduped against the student's real
 * topikVocabularyProgress by UserProfileContext.addTopikVocabularyWord, so
 * the same word never appears twice even if saved from multiple sessions.
 */
export function TopikReadingVocabularyCard({
  vocabulary,
  savedWords,
  onSave,
}: {
  ...
  onSave: (item: TopikReadingVocabularyItem) => void;
}) {
  const { t } = useLanguage();
