...
import type { TopikReadingFeedback } from "@/types/topik-reading";
...
/** Mirrors ReadingSkillBreakdown exactly. Every skill score/explanation is
 * computed purely from this session's real question results (or real
 * timing, for "speed") — see lib/topik/reading-feedback.ts. Never
 * randomized or templated independent of the data. The card title reuses
 * t.reading.skillBreakdown.title; the description uses TOPIK's own
 * t.topik.reading.skillBreakdownDescription so it never says "DELF". */
export function TopikReadingSkillBreakdown({ feedback }: { feedback: TopikReadingFeedback }) {
  const { t } = useLanguage();
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{t.topik.reading.skillBreakdownDescription}</CardDescription>
      </CardHeader>
...
/** Mirrors ReadingStrategyCard exactly (AI Reading Strategy). Every
 * insight is gated behind a real computed condition, see
 * lib/topik/reading-feedback.ts#buildStrategyInsights. The readiness label
 * uses TOPIK's own t.topik.reading.readinessLabel. */
export function TopikReadingStrategyCard({ feedback }: { feedback: TopikReadingFeedback }) {
  const { t } = useLanguage();
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1 rounded-2xl bg-primary-50 p-4">
          <span className="text-xs font-medium text-primary-700">{t.topik.reading.readinessLabel}</span>
