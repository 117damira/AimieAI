...
import { Clock, FileText, Type, Award, Target, Layers } from "lucide-react";
...
/** Mirrors ReadingLevelDashboard exactly, driven by TopikReadingLevelConfig
 * instead of DelfReadingLevelConfig. Reuses the exam-agnostic t.reading.home
 * strings for every stat except "current level", which uses TOPIK's own
 * t.topik.reading.currentLevelLabel override so it never says "DELF". */
export function TopikReadingLevelDashboard({ config }: { config: TopikReadingLevelConfig }) {
  const { t } = useLanguage();
  const h = t.reading.home;
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center gap-2.5">
          <Badge variant="primary">
            ...
            {t.topik.reading.currentLevelLabel}
          </Badge>
        ...
        <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Stat icon={FileText} label={h.passages} value={`${config.passageCountLabel} ${h.passagesUnit}`} />
          <Stat icon={Type} label={h.maxWordsPerPassage} value={`${config.maxWordsPerPassage} ${h.wordsUnit}`} />
