...
/** Mirrors ReadingProgressComparison exactly. Always compares the
 * just-completed session against the real previous TOPIK Reading session
 * stored in profile.topikStats.readingSessionHistory, computed by the
 * shared, exam-agnostic lib/reading/stats.ts#computeReadingProgressComparison
 * — never a fabricated baseline. */
export function TopikReadingProgressComparison({ comparison }: { comparison: ReadingProgressComparisonData }) {
  const { t } = useLanguage();
