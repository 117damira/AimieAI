...
import { classifyTopikPace } from "@/lib/topik/reading-timing";
import { useLanguage } from "@/lib/i18n/LanguageContext";
import type { TopikReadingResult } from "@/types/topik-reading";
...
/** Mirrors ReadingResultsSummary exactly. Reuses the exam-agnostic
 * t.reading.results.* strings for everything except the recommended-time
 * stat, which uses TOPIK's own t.topik.reading.recommendedTimeLabel so it
 * never says "Recommended DELF time". */
export function TopikReadingResultsSummary({ result }: { result: TopikReadingResult }) {
  const { t } = useLanguage();
  const r = t.reading.results;
  const isOfficialExam = result.mode === "full-exam";
  const paceTier = classifyTopikPace(result.timing.paceRatio);
      ...
      <CardContent className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-border p-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Stat icon={Gauge} label={t.topik.reading.recommendedTimeLabel} value={`${result.timing.recommendedMinutes} min`} />
          </div>
        </div>
      </CardContent>
    </Card>
