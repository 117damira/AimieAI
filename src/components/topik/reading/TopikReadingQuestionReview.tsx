...
import type { TopikReadingPassage, TopikReadingQuestion } from "@/types/topik-reading";

const DIFFICULTY_EMOJI = { easy: "🟢", medium: "🟡", hard: "🔴" } as const;

/**
 * Mirrors ReadingQuestionReview exactly, including the "Highlight Evidence"
 * feature — it reuses lib/reading/highlight.ts#highlightEvidence directly,
 * since that helper is a generic, exam-agnostic substring-matcher (no DELF
 * content/types involved) that just wraps the real supporting Korean
 * sentence(s) in <mark>, never revealing the answer outright.
 */
export function TopikReadingQuestionReview({
  question,
  questionNumber,
  selectedOptionIds,
  isCorrect,
  ...
  question: TopikReadingQuestion;
  questionNumber: number;
  selectedOptionIds: string[];
  isCorrect: boolean;
  pointsEarned: number;
  pointsPossible: number;
  passage: TopikReadingPassage | undefined;
}) {
  const { t } = useLanguage();
