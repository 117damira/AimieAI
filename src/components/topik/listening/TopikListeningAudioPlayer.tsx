...
import { pickVoice, TOPIK_SPEECH_LOCALE } from "@/lib/utils/voice";
 ...
 * and the audio is always real, audible synthesized Korean speech (see
 * TOPIK_SPEECH_LOCALE in lib/utils/voice.ts).
 *
 * Mirrors components/listening/ListeningAudioPlayer.tsx exactly, with "fr"
 * swapped for TOPIK_SPEECH_LOCALE ("ko") and the fallback utterance lang
 * swapped from "fr-FR" to "ko-KR".
 *
 ...
 */
export function TopikListeningAudioPlayer({ transcript }: { transcript: string }) {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    pickVoice(TOPIK_SPEECH_LOCALE).then((voice) => {
      if (cancelled) return;
