...
/** Mirrors components/listening/ListeningLevelDashboard.tsx. The "Current
 * Level" badge label comes from t.topik.listening.currentLevelLabel (says
 * "TOPIK", not "DELF"); every stat row below reuses t.listening.home
 * verbatim since those labels ("Listening duration", "Recordings", "Max
 * recording length", "Maximum score", "Minimum passing score") never
 * mention DELF. */
export function TopikListeningLevelDashboard({ config }: { config: TopikListeningLevelConfig }) {
  const { t } = useLanguage();
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center gap-2.5">
          <Badge variant="primary">
            ...
            {t.topik.listening.currentLevelLabel}
          </Badge>
