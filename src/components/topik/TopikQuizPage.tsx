...
import { generateTopikVocabularyQuiz } from "@/lib/quiz/generateTopikQuiz";
import { useLanguage } from "@/lib/i18n/LanguageContext";
import { useUserProfile } from "@/lib/profile/UserProfileContext";
import { cn } from "@/lib/utils/cn";

/** TOPIK equivalent of DelfQuizPage — identical structure and reused
 * generic t.quiz.* copy, sourced from profile.topikVocabularyProgress and
 * recording completion into profile.topikStats via recordTopikQuizCompletion. */
export function TopikQuizPage() {
  const { t, language } = useLanguage();
  const { profile, recordTopikQuizCompletion } = useUserProfile();
  ...
  const questions = useMemo(
    () => generateTopikVocabularyQuiz(profile?.topikVocabularyProgress ?? [], language),
    [profile?.topikVocabularyProgress, language]
  );
